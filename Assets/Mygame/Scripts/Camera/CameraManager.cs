...
public class CameraManager : MonoBehaviour
{
    ...
    List<(InputType , Action)> getInputCallBack = new();
    List<(InputType, Action)> menuInputCallBack = new();

    private void Awake()
    {
        getInputCallBack.Add((new InputType(InputModeType.InGame, InputActionType.Zoom, ExecuteType.Enter, UpdateMode.Update), ZoomStart));
        getInputCallBack.Add((new InputType(InputModeType.InGame, InputActionType.Zoom, ExecuteType.Exit, UpdateMode.Update), ZoomEnd));
        getInputCallBack.Add((new InputType(InputModeType.InGame, InputActionType.Cancel, ExecuteType.Enter, UpdateMode.Update), GetCancel));
        menuInputCallBack.Add((new InputType(InputModeType.Menu, InputActionType.Cancel, ExecuteType.Enter, UpdateMode.Update), GetCancel));
    }
    void OnEnable()
    {
